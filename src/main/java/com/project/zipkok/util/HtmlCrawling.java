...
import com.project.zipkok.repository.RealEstateImageRepository;
...
public class HtmlCrawling {
    ...
    private final RealEstateImageRepository realEstateImageRepository;
...
//             위도, 경도 parsing
            ...
            // 상세정보 parsing
            detail = driver.findElement(By.cssSelector("div.styled__MemoContainer-sc-413snh-0.ehElXP")).getText();
            System.out.println("상세정보 길이 : " + detail.length());
...
//            System.out.println("=================저장완료===================\n\n");
            ...
            //이미지 url parsing
            List<WebElement> elements = driver.findElements(By.cssSelector("div[class^='styled__Photo-sc-173484h-2']"));
            System.out.println("사이즈 : " + elements.size());

            for(WebElement element : elements){
                // ::after 값을 가져오기 위해 JavaScript를 실행
                String script = "return window.getComputedStyle(arguments[0], ':after').getPropertyValue('background-image');";
                String backgroundImage = (String) ((JavascriptExecutor) driver).executeScript(script, element);

                backgroundImage = backgroundImage.substring(backgroundImage.indexOf("\"")+1, backgroundImage.length()-2);

                System.out.println("content : " + backgroundImage);

                RealEstateImage realEstateImage = RealEstateImage.builder()
                        .imageUrl(backgroundImage)
                        .realEstate(realEstate)
                        .status("active")
                        .build();

                this.realEstateImageRepository.save(realEstateImage);
